...
SPI_IO_CS_HIGH_CFG (as defined)
    */
    ...
    if (gpio_get_function(ioPin) != GPIO_FUNC_SIO) {
        // IOConfigGPIO means software-controlled GPIO.  This transition is
        // required when serial 4-way temporarily takes a motor pin away from
        // PIO DShot.
        gpio_init(ioPin);
    }
